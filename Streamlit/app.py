...
def get_pdf_file_hash(file_path: str) -> str:
    """计算PDF文件的哈希值"""
    with open(file_path, 'rb') as f:
        file_hash = hashlib.md5(f.read()).hexdigest()
    return file_hash

def load_pdf_topic_from_cache(pdf_hash: str) -> Optional[Dict]:
    """从缓存加载PDF主题信息"""
    cache_key = f"pdf_topic_{pdf_hash}"
    if cache_key in st.session_state:
        return st.session_state[cache_key]
    return None

def save_cache_pdf_topic(pdf_hash: str, pdf_info: Dict):
    """保存PDF主题信息到缓存"""
    cache_key = f"pdf_topic_{pdf_hash}"
    st.session_state[cache_key] = pdf_info

class LayeredMemoryManager:
    """简化版分层记忆管理器"""
    def __init__(self, llm, short_term_max_rounds=3):
        self.llm = llm
    ...
    def add_message(self, role: str, content: str):
        ...
        # 保持短期记忆长度
        if len(self.short_term_memory) > self.short_term_max_rounds * 2:
            # 将旧的对话转移到长期记忆
            if len(self.short_term_memory) >= 2:
                old_msg = self.short_term_memory.pop(0)
                self.long_term_memory.append(old_msg)
    ...
    def get_combined_memory(self) -> str:
        ...
        if self.long_term_memory:
            combined.append("【历史对话摘要】")
            for msg in self.long_term_memory[-3:]:  # 只取最近3条长期记忆
                role = "用户" if msg["role"] == "user" else "助手"
                combined.append(f"{role}：{msg['content'][:100]}...")
        
        # 添加短期记忆
        if self.short_term_memory:
            combined.append("【最近对话】")
            for msg in self.short_term_memory:
                role = "用户" if msg["role"] == "user" else "助手"
                combined.append(f"{role}：{msg['content']}")
        
        return "\n".join(combined) if combined else "暂无对话历史"
...
def init_session_state():
    ...
    if 'qa_system_initialized' not in st.session_state:
        st.session_state.qa_system_initialized = False
    if 'qa_func' not in st.session_state:
        st.session_state.qa_func = None
    if 'clear_history_func' not in st.session_state:
        st.session_state.clear_history_func = None
    if 'clear_all_cache_func' not in st.session_state:
        st.session_state.clear_all_cache_func = None
    ...
    if 'relevance_cache' not in st.session_state:
        st.session_state.relevance_cache = {}
    if 'llm_cache' not in st.session_state:
        st.session_state.llm_cache = InMemoryCache()
        set_llm_cache(st.session_state.llm_cache)
...
def load_and_split_pdf(pdf_path):
    ...
    split_docs = semantic_split(
        docs=raw_docs,
        max_chunk_len=st.session_state.config["max_chunk_size"],
        min_chunk_len=st.session_state.config["min_chunk_size"]
    )
    
    return split_docs, raw_docs
...
def build_qa_chain(uploaded_file=None, pdf_path=None):
    # 确定PDF路径
    if uploaded_file:
        # 保存上传的文件到临时目录
        ...
        raise ValueError("必须提供PDF文件路径或上传的文件")
    
    st.session_state.config["pdf_path"] = pdf_path
    
    # 1. 加载PDF并初始化向量库
    with st.spinner("📄 正在加载PDF文档..."):
        split_docs, raw_docs = load_and_split_pdf(pdf_path)
    ...
    with st.spinner("🔧 正在初始化向量数据库..."):
        ...
        vector_db = Chroma.from_documents(
            ...
            persist_directory=st.session_state.config["chroma_dir"]
        )
        # 创建向量检索器
        retriever = vector_db.as_retriever(search_kwargs={"k": st.session_state.config["retrieve_top_k"]})

    # 加载/提取PDF基础信息（优先从缓存加载）
    pdf_hash = get_pdf_file_hash(pdf_path)
    pdf_info = load_pdf_topic_from_cache(pdf_hash)
    if not pdf_info:
        with st.spinner("📊 正在提取PDF核心信息..."):
            pdf_info = get_pdf_basic_info(raw_docs, llm)
        save_cache_pdf_topic(pdf_hash, pdf_info)

    # ========== BM25 关键词检索器初始化（解决向量检索对关键词不敏感问题） ==========
    with st.spinner("🔍 正在初始化关键词检索器..."):
        # 1. 提取所有文档片段的文本和元数据
    ...
    memory_manager = LayeredMemoryManager(
        llm=llm,
        short_term_max_rounds=st.session_state.config["short_term_max_rounds"]
    )
    ...
    def pdf_page_search_tool(page_num: str) -> str:
        ...
        try:
            ...
            # 重新加载PDF文档
            all_docs, _ = load_and_split_pdf(pdf_path)
            page_content = []
    ...
    def is_question_relevant(question):
        ...
        # 1. 优先查自定义缓存，提升效率
        if q_normalized in st.session_state.relevance_cache:
            return st.session_state.relevance_cache[q_normalized]
        ...
        try:
            ...
            # 存入缓存，后续复用
            st.session_state.relevance_cache[q_normalized] = (is_rel, reason)
    ...
    def qa_function(question):
        ...
        # 1. 优先查最终问答结果缓存，提升效率
        if q_normalized in st.session_state.qa_cache:
            return st.session_state.qa_cache[q_normalized]
        
        # 2. 判断问题相关性，过滤无关问题
        is_relevant, reason = is_question_relevant(question_clean)
        if not is_relevant:
            result = (f"❌ 抱歉，我仅能回答与PDF相关的问题哦～（原因：{reason}）", [])
            st.session_state.qa_cache[q_normalized] = result
            return result
...
def main():
    ...
    with st.sidebar:
        ...
        # 初始化按钮
        if uploaded_file and not st.session_state.qa_system_initialized:
            if st.button("🚀 初始化问答系统", type="primary", use_container_width=True):
                with st.spinner("正在初始化问答系统，请稍候..."):
                    try:
                        # 初始化模型（移到函数内部，避免重复初始化）
                        global llm, embeddings, reranker
                        
                        # 1. 嵌入模型初始化
                        embeddings = HuggingFaceEmbeddings(
                            model_name=st.session_state.config["embedding_path"],
                        ...
                        llm = ChatOpenAI(
                            ...
                            temperature=st.session_state.config["llm_temperature"],
                            api_key=st.session_state.config["qwen_api_key"],
                            base_url=st.session_state.config["qwen_base_url"],
                            cache=True  # 开启LLM原生缓存兜底
                        )

                        # 3. 重排序模型初始化
                        reranker = HuggingFaceCrossEncoder(
                            model_name=st.session_state.config["rerank_path"],
                            model_kwargs={"device": "cpu"}
                        )
                        
                        # 构建问答链
                        st.session_state.qa_func, st.session_state.clear_history_func, st.session_state.clear_all_cache_func = build_qa_chain(uploaded_file=uploaded_file)
                        st.session_state.qa_system_initialized = True
        ...
        if st.session_state.qa_system_initialized:
            st.success("✅ 系统已就绪")
            st.info(f"📄 当前PDF: {uploaded_file.name}")
        else:
            st.warning("⚠️ 请上传PDF并初始化系统")
        ...
        with col1:
            if st.button("🗑️ 清空对话", use_container_width=True):
                if st.session_state.clear_history_func:
                    result = st.session_state.clear_history_func()
                    st.session_state.conversation_history = []
                    st.success(result)
                    st.rerun()
                else:
        ...
        with col2:
            if st.button("🧹 清空缓存", use_container_width=True):
                if st.session_state.clear_all_cache_func:
                    result = st.session_state.clear_all_cache_func()
                    st.success(result)
                    st.rerun()
                else:
                    st.warning("系统未初始化")
    ...
    # 状态显示
    if not st.session_state.qa_system_initialized:
        st.info("👈 请在侧边栏上传PDF文件并初始化系统")
        ...
        for i, conv in enumerate(st.session_state.conversation_history):
            with st.chat_message("user"):
                st.markdown(f"**Q{i+1}:** {conv['question']}")
            
            with st.chat_message("assistant"):
                st.markdown(conv['answer'])
                if conv['sources']:
                    st.caption(f"📎 来源: {', '.join(conv['sources'])}")
        ...
        with col1:
            question = st.text_input(
                "输入您的问题:",
                placeholder="例如：这份PDF讲了什么？营收增速是多少？",
        ...
        with col1:
            if st.button("📋 文档主题", use_container_width=True):
                question = "这份PDF讲了什么？"
                submit_btn = True
        with col2:
            if st.button("🔍 查具体内容", use_container_width=True):
                question = "营收增速是多少？"
                submit_btn = True
        with col3:
            if st.button("📄 按页码查", use_container_width=True):
                question = "第5页内容是什么？"
                submit_btn = True
        ...
        if submit_btn and question:
            ...
            with st.chat_message("assistant"):
                with st.spinner("🤔 正在思考..."):
                    try:
                        answer, sources = st.session_state.qa_func(question)
...
if __name__ == "__main__":
    ...
    os.makedirs(DEFAULT_CONFIG["chroma_dir"], exist_ok=True)
    
    # 检查模型文件
    if not os.path.exists(DEFAULT_CONFIG["embedding_path"]):
        st.warning(f"嵌入模型路径不存在: {DEFAULT_CONFIG['embedding_path']}")
    
    if not os.path.exists(DEFAULT_CONFIG["rerank_path"]):
        st.warning(f"重排序模型路径不存在: {DEFAULT_CONFIG['rerank_path']}")
